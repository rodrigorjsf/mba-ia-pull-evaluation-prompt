...
# --- version selection (v2 is the only evaluated prompt) --------------------
...
def test_pull_prompt_builds_versioned_hub_name():
    ...
    with patch("run_experiment.hub.pull", return_value=MagicMock()) as mock_pull:
        run_experiment.pull_prompt("rodrigorjsf", "v2")
    mock_pull.assert_called_once_with("rodrigorjsf/bug_to_user_story_v2")
...
def test_main_rejects_invalid_version():
    """An unknown version — and the purged v0 baseline — are rejected before any
    experiment is created. The deliverable evaluates only v2."""
    for version in ("v9", "v0"):
