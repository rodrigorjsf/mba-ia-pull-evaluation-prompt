...
or `create_feedback`, so the dashboard has **no scored Experiment** and no comparison view.
...
- targets prompts **pulled from the Hub** (`v0` and `v2`) over the existing eval dataset,
- reuses the `InMemoryRateLimiter` throttle (`install_rate_limiter`) so the Gemini free tier
  does not 429.

Result: **one Experiment per prompt** with per-example feedback scores → the Experiments tab
plus the native **Comparison View** (the SPEC's "tabela comparativa", produced by LangSmith).
...
- **`create_feedback` on the traces** produced by `evaluate.py` — rejected: no native
  Experiment object, so no Comparison View.
- **Keep traces + terminal only** (ADR-0001) — rejected: the scores never appear *in the
  dashboard*, which is what §3 asks for.

## Consequences

- The dashboard shows scored Experiments + a Comparison View + traces of ≥ 3 examples.
- The judge stays **Gemini free** (`gemini-3.1-flash-lite`), so Experiment scores match the
  `evaluate_throttled.py` terminal run — one consistent number across all evidence.
